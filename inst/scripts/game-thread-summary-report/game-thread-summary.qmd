...
file_list <- list.files(path = paste0(path_start, params$date, "/"), pattern = "*.csv")
# comments_ <- lapply(paste0(path_start, date, "/", file_list), read_csv)
...
thread_name <- file_list[grep(paste0("@ ", params$home_team, " \\("), file_list)]

if(length(thread_name) == 0) { 
  stop("No matching game thread data found! Try again.")
}
#print("Game thread data found!")
comments_ <- read_csv(paste0(path_start, params$date, "/", thread_name)) |>
  mutate(id = str_remove(thread_name, ".csv")) |>
  select(!`...1`) |>
  rename(title = id, body = `0`, flair = `1`, time_unix = `2`, score = `3`)
#print("Game thread data loaded successfuly!")
...
sched <- cfbfastR::espn_cfb_schedule(year = params$yr,
                                     week = params$wk) |>
  select(game_date, home_team_location, start_date) |>
  mutate(
...
away_team_color <- team_info[which(team_info$school == away_team_name), ]$alt_color
...
# `r str_remove_all(bowl_name, "\\[Game Thread\\]|.csv") |> trimws()`
...
thread_timeline_data <- comments |> 
  ...
  count(
    ...
    title,
    faction,
  # DISABLED DUE TO LACK OF BOWL GAME SCHEDULES
    # start_date, end_date
    ) # |> 
...
thread_timeline_data |>
  filter(n > 10) |>   # ENABLED DUE TO LACK OF BOWL GAME SCHEDULES
